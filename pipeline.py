...
class IncidentRecord:
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            "title": self.title,
            # "url": self.url,  # Column doesn't exist in database
...
Return only valid JSON."""
    ...
    async def analyze_image_claim(self, title: str, location: str, photo_base64: str) -> Dict:
        """Analyze image to verify user's claim using Groq's vision model.
        Returns {"valid": bool, "analysis": str, "detected_issues": [...], "confidence": int}
        """
        if not self.llm_api_url or not self.llm_api_key:
            return {"valid": False, "analysis": "LLM not configured"}
        
        try:
            # Use Groq's vision model (llama-4-scout-17b-16e-instruct)
            prompt = f"""You are an expert road incident analyst. Analyze this image to verify the user's claim.

User claims: "{title}"
Location: {location}

Analyze the image and return ONLY a valid JSON object (no markdown, no extra text):

{{
  "valid": true or false,
  "detected_issues": ["list", "of", "visible", "issues"],
  "analysis": "2-3 sentence description of what you see in the image",
  "confidence": <integer 0-100>
}}

Guidelines:
- Set valid=true ONLY if the image clearly shows a road/traffic incident matching the claim
- Set valid=false if: image is blurry, shows no incident, doesn't match the claim, or is unrelated
- detected_issues: list specific problems visible (e.g., "damaged vehicle", "blocked road", "oil spill")
- confidence: 0-100 score (0=no incident visible, 100=clear incident matching claim)
- Be strict: require clear visual evidence

Examples:
- Claim "car crash" + image shows damaged cars → valid=true, confidence=90
- Claim "traffic jam" + image shows empty road → valid=false, confidence=10
- Claim "oil spill" + image shows liquid on road → valid=true, confidence=85
- Blurry/unclear image → valid=false, confidence=20"""
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if r.status_code != 200:
                    print(f"[LLM_ANALYZE] API error {r.status_code}: {r.text}")
                    return {"valid": False, "analysis": f"API error: {r.status_code}"}
                ...
                if "detected_issues" not in parsed:
                    parsed["detected_issues"] = []
                if "analysis" not in parsed:
                    parsed["analysis"] = "Analysis completed"
                
                # If confidence < 50%, mark as invalid
                if parsed.get("confidence", 50) < 50:
                    parsed["valid"] = False
                
                print(f"[LLM_ANALYZE] Result: valid={parsed['valid']}, confidence={parsed['confidence']}")
                return parsed
                
        except json.JSONDecodeError as e:
            print(f"[LLM_ANALYZE] JSON parse error: {e}")
            print(f"[LLM_ANALYZE] Raw response: {raw_text[:200]}")
            return {
                "valid": False,
    ...
    async def generate_incident_fields_from_image(self, title: str, location: str, image_analysis: Dict) -> Dict:
        """Generate all incident fields (summary, reason, priority, actions, skills, steps, volunteers) from title, location, and image analysis.
        Returns complete dict ready for Supabase.
        """
        if not self.llm_api_url or not self.llm_api_key:
            return {}
        
        try:
            detected = ", ".join(image_analysis.get("detected_issues", ["unknown"]))
            analysis_text = image_analysis.get("analysis", "Incident reported")
            
            prompt = f"""Based on this road incident report in Pune, generate comprehensive incident details.
...
{{
  "summary": "1-2 sentence detailed description",
  "reason": "crash|collision|fire|flood|closure|breakdown|accident|construction|landslide|spill|debris|weather",
  "priority": "low|medium|high|critical",
  "actions_needed": ["action1", "action2", "action3"],
  "required_skills": ["skill1", "skill2"],
...
Guidelines:
- priority: critical if injuries/hazmat, high if closure/major impact, medium if minor, low if minimal
- actions_needed: 3-4 specific volunteer actions (Clear debris, Direct traffic, First aid, etc.)
- required_skills: Pick from [first_aid, traffic_control, cleanup, documentation, communication, heavy_equipment]
- estimated_volunteers: 1-10 based on severity
- Return ONLY valid JSON, no other text."""
            ...
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                if r.status_code == 200:
                    ...
                    parsed = json.loads(result)
                    return {
                        "summary": parsed.get("summary", "Road incident reported"),
                        "reason": parsed.get("reason", "accident"),
                        "priority": parsed.get("priority", "medium"),
                        "actions_needed": parsed.get("actions_needed", []),
                        "required_skills": parsed.get("required_skills", []),
                        "resolution_steps": parsed.get("resolution_steps", []),
...
class IncidentPipeline:
    ...
    async def process_article(self, article: Dict) -> Optional[IncidentRecord]:
        ...
        try:
            ...
            # Fix: Only use location if it's a valid place name, not a URL
            location_value = None
            if location and not location.startswith(('http://', 'https://', 'www.')):
            ...
            try:
                async with httpx.AsyncClient(timeout=10.0) as client:
                    ...
                    if r.status_code in (200, 201):
                        ...
                        print(f"[ERROR] Supabase insert failed with status {r.status_code}")
